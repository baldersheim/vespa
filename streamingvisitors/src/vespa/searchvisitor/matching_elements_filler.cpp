...
public:
    SubFieldTerm(vespalib::string field_name, vsm::FieldIdT id, const QueryTerm* term) noexcept
        : _field_name(std::move(field_name)),
          _id(id),
          _term(term)
    {
    }
    const vespalib::string& get_field_name() const noexcept { return _field_name; }
    const QueryTerm& get_term() const noexcept { return *_term; }
    vsm::FieldIdT get_id() const noexcept { return _id; }
};
...
class Matcher
{
    ...
    void add_matching_elements(const vespalib::string& field_name, std::optional<FieldIdT> field_id, uint32_t doc_lid, const HitList& hit_list, MatchingElements& matching_elements);
...
{
    const QueryTermData& qtd = dynamic_cast<const QueryTermData &>(multi_term.getQueryItem());
    ...
    for (size_t i = 0; i < num_fields; ++i) {
        ...
        if (fields.has_struct_field(field_name)) {
            field = &fields.get_enclosing_field(field_name);
...
{
    const QueryTermData& qtd = dynamic_cast<const QueryTermData &>(query_term.getQueryItem());
    auto& td = qtd.getTermData();
    auto num_fields = td.numFields();
    for (size_t i = 0; i < num_fields; ++i) {
        auto field_id = td.field(i).getFieldId();
        ...
        auto &field_name = field_info->name();
        if (fields.has_struct_field(field_name)) {
            _sub_field_terms.emplace_back(fields.get_enclosing_field(field_name), field_id, &query_term);
        }
        if (fields.has_field(field_name)) {
            _sub_field_terms.emplace_back(field_name, field_id, &query_term);
        }
    }
}
...
void
Matcher::add_matching_elements(const vespalib::string& field_name, std::optional<FieldIdT> field_id, uint32_t doc_lid, const HitList& hit_list, MatchingElements& matching_elements)
{
    _elements.clear();
